...
void main(void) {
	...
	vec4 texOne = texture2D(sceneTexture, vec2(vTextureCoord.s, vTextureCoord.t));
	// vec4 texTwo = texture2D(dustTexture, vec2(vTextureCoord.s, vTextureCoord.t));

	finalColor.a = texOne.a;

	float n = snoise(vec3(.5 * vVertexPos * (1.0 + sin(.5 * 3.14))));

	if (texOne.r > 0.1)
		finalColor.rgb = texOne.rgb + n;
	else
		finalColor.rgb = texOne.rgb;

	// if (texTwo.r > .8)
	// 	finalColor.rgb = texTwo.rgb * texOne.rgb * 2.0;
	// else
	// 	finalColor.rgb = texOne.rgb;
	
	// finalColor.a = texOne.a * texTwo.a;
